...
func NewConn(connType ConnType, options *Options, logger logging.Logger) (*Conn, error) {
	...
	netMap.SelfNode = &tailcfg.Node{
		ID:         tailcfg.NodeID(options.NodeID),
		Key:        nodePublicKey,
		Addresses:  options.Addresses,
		AllowedIPs: options.Addresses,
	}

	wireguardMonitor, err := monitor.New(logger.TailscaleDebugLogger("ts-wgmonitor"))
	if err != nil {
		return nil, xerrors.Errorf("create wireguard link monitor: %w", err)
	}

	dialer := &tsdial.Dialer{
		Logf: logger.TailscaleDebugLogger("ts-dialer"),
	}
	wireguardEngine, err := wgengine.NewUserspaceEngine(logger.TailscaleDebugLogger("ts-wgengine"), wgengine.Config{
	...
	netStack, err := netstack.Create(
		logger.TailscaleDebugLogger("ts-netstack"), tunDevice, wireguardEngine, magicConn, dialer, dnsManager)
	if err != nil {
	...
	wireguardEngine.SetFilter(filter.New(netMap.PacketFilter, localIPs, logIPs, nil, logger.TailscaleDebugLogger("ts-packet-filter")))
	...
	wireguardEngine.SetStatusCallback(func(s *wgengine.Status, err error) {
		...
		server.logger.Debugf("wireguard status: %s - %v", string(buf), err)
		if err != nil {
			return
		}
	...
	wireguardEngine.SetNetInfoCallback(func(ni *tailcfg.NetInfo) {
		buf, _ := json.Marshal(ni)
		server.logger.Debugf("netinfo callback: %s", string(buf))
		server.lastMutex.Lock()
...
func (c *Conn) SetDERPMap(derpMap *tailcfg.DERPMap) {
	...
	c.logger.Debugf("updating derp map: %s", string(buf))
...
func (c *Conn) RemoveAllPeers() error {
	...
	cfg, err := nmcfg.WGCfg(c.netMap, c.logger.TailscaleDebugLogger("wgconfig"), netmap.AllowSingleHosts, "")
	if err != nil {
...
func (c *Conn) UpdateNodes(nodes []*Node) error {
	...
	for _, node := range nodes {
		...
		c.logger.Debugf("adding node: %v", string(buf))
	...
	netMapCopy := *c.netMap
	c.wireguardEngine.SetNetworkMap(&netMapCopy)
	cfg, err := nmcfg.WGCfg(c.netMap, c.logger.TailscaleDebugLogger("ts-wgconfig"), netmap.AllowSingleHosts, "")
	if err != nil {
		return xerrors.Errorf("update wireguard config: %w", err)
	}
	err = c.wireguardEngine.Reconfig(cfg, c.wireguardRouter, &dns.Config{}, &tailcfg.Debug{})
...
func (c *Conn) UpdateNodesClear(nodes []*Node) error {
	c.logger.Debugf("(conn: %d) updating nodes: %d", c.netMap.SelfNode.ID, len(nodes))
	for _, n := range nodes {
		c.logger.Debugf("(conn: %d) adding node: %d", c.netMap.SelfNode.ID, int64(n.ID))
	}

	err := c.RemoveAllPeers()
	if err != nil {
		return xerrors.Errorf("remove all peers: %w", err)
	}

	err = c.UpdateNodes(nodes)
	if err != nil {
...
// Connects the connection to the provided coordinator connection
// and processes network updates automatically.
func (c *Conn) ConnectToCoordinator(conn net.Conn) <-chan error {
	...
	// Receive nodes that want to connect from the coordinator and update the connection with their keys
	go func() {
		decoder := json.NewDecoder(conn)
		for {
			c.logger.Debugf("(coordinator handler) waiting for nodes")
			var nodes []*Node
			err := decoder.Decode(&nodes)
			if err != nil {
				c.logger.Errorf("(coordinator handler) exiting")
				sendErr(xerrors.Errorf("read: %w", err))
				return
			}
			c.logger.Debugf("(coordinator handler) received %d nodes", len(nodes))
			for _, n := range nodes {
				c.logger.Debugf("(coordinator handler) adding node as peer: %d - %+v", int64(n.ID), n)
			}

			if c.connType == ConnTypeAgent {
				// agent connections are 1:N, so we can just add the peers
				err = c.UpdateNodes(nodes)
			} else {
				// server connections are 1:1, so we clear all the peers to ensure that the connection is clean and up to date
				err = c.UpdateNodesClear(nodes)
			}
			if err != nil {
	...
	// Send the current node to the coordinator so connections can be established
	c.SetNodeCallback(func(node *Node) {
...
func (c *Conn) sendNode() {
	...
	go func() {
		buf, _ := json.Marshal(node)
		c.logger.Debugf("sending node: %v", string(buf))
		nodeCallback(node)
		c.lastMutex.Lock()
		c.nodeSending = false
		if c.nodeChanged {
...
func (c *Conn) forwardTCPToLocal(conn net.Conn, port uint16) {
	...
	if err != nil {
		c.logger.Debugf("dial local port: %d - %v", port, err)
		return
	}
	defer server.Close()
	...
	if err != nil {
		c.logger.Debugf("proxy connection closed with error: %v", err)
	}
	c.logger.Debugf("forwarded connection closed: %s", dialAddrStr)
}
